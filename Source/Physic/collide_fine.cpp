...
{
	//Matrix 4x4 offset, original is Identity
...
bool IntersectionTests::ShpereAndRay(Ray & ray,  CollisionBox &box, CollisionSphere &sphere)
{
	Vector3 m = ray.original - sphere.body->getPosition();
	float b = m * ray.direction;
	float c = m * m  - sphere.radius - sphere.radius;
...
bool IntersectionTests::boxAndRay(Ray & ray,  CollisionBox &one, CollisionBox &two)
{
    Vector3 m = ray.original - two.body->getPosition();
	float b = m * ray.direction;
	float c = m * m  - two.halfSize.x * two.halfSize.x;
	if (c > 0.0f && b > 0.0f) return 0;
    float discr = b*b - c;
	if (discr < 0.0f) return 0;
	float t = -b - sqrt(discr);
    ...
    )
{
    ...
    for (unsigned i = 0; i < 8; i++) {
		...
		//Trans corner position to box local position

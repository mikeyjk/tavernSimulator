...
Personality::Personality()
{
	...
	HVDynamics.initHumanValues(&HVDynamics, 0);
...
void Personality::update(double deltaTime)
{
	...
	if(dTime >= 5)
	{
		...
		//update strength AT MOST THIS WILL - 25
		HVDynamics.Phys.Strength = HVThreshold.Phys.Strength - (((HVDynamics.Vit.Hunger /2) *  (HVDynamics.Vit.Thirst /2)) / 100);

		//update agility AT MOST THIS WILL - 25
		HVDynamics.Phys.Agile = HVThreshold.Phys.Agile - (((HVDynamics.Vit.Hunger /2) * (HVDynamics.Vit.Tiredness /2)) / 100);

		//if intoxicated then update agility again
		if(HVDynamics.Vit.Toxication > 20)
		{//if the AI is beginning to get intoxicated they will lose agility more rapidly (speed and dodging) AT MOST THIS WILL - 50
			HVDynamics.Phys.Agile = HVThreshold.Phys.Agile - ((((HVDynamics.Vit.Hunger /2) + (HVDynamics.Vit.Toxication /2)) * (HVDynamics.Vit.Tiredness /2)) / 100);
		}
		determineEmotion();
		dTime = 0;
	}

	if(dTime == 0)//has been reset
	{
		printValues();
	}
...
enEmotion Personality::getEnEmotion()
{
...
std::vector<enTag> Personality::getNeeds()
{
	std::vector<enTag> needs;

	if(HVDynamics.Vit.Hunger > HVThreshold.Vit.Hunger)
	{
		needs.push_back(enTag::eat);
	}

	if(HVDynamics.Vit.Thirst > HVThreshold.Vit.Thirst)
	{
		needs.push_back(enTag::drink);
	}

	if(HVDynamics.Vit.Tiredness > HVThreshold.Vit.Tiredness)
	{
		needs.push_back(enTag::sleep);
	}

	if(HVDynamics.Vit.Bladder > HVThreshold.Vit.Bladder)
	{
		needs.push_back(enTag::toilet);
	}
	...
	if(HVDynamics.Ment.Boredom > HVThreshold.Ment.Boredom )
	{
		needs.push_back(enTag::wander);
	}

	if(HVDynamics.Ment.Talk > HVThreshold.Ment.Talk )
	{
		needs.push_back(enTag::talk);
	}

	if(HVDynamics.Ment.Curiosity > HVThreshold.Ment.Curiosity )
	{
		needs.push_back(enTag::investigate);
	}
